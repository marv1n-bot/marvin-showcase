...
export function PortfolioGrid({ portfolio }: { portfolio: PortfolioItem[] }) {
  return (
    <section id="portfolio" className="border-t border-line px-6 py-16 sm:px-10 lg:px-16">
      <p className="font-display text-xs uppercase tracking-[0.4em] text-cyan">Greatest hits</p>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
        {portfolio.map((item) => (
          <a
            ...
            className="group rounded-lg border border-line bg-panel p-6 transition hover:border-cyan-dim hover:bg-panel-light"
          >
            <span className="font-display text-lg font-medium">{item.name}</span>
            <p className="mt-2 text-sm text-fg-dim">{item.description}</p>
            <span className="mt-4 inline-flex items-center gap-1 font-display text-xs uppercase tracking-wide text-cyan">
              {item.linkLabel}
